...
final class ChatScreenViewController: UIViewController {
    ...
    private func sendMessage() {
        serviceProvider.aiManager.getDialogueResponse(
            model,
            role: ChatRole(rawValue: serviceProvider.settingStorage.selectedRoleIndex) ?? .dialog
        ) { [weak self] result in
    ...
    private func saveMessage(_ message: MessageModel) {
        do {
            ...
            try  serviceProvider.coreDataManager.saveChatMessage(message)
            addMessageToTable()
        } catch {
            print(error)
        }
